...
namespace Doctor
{
    public partial class FormTblOperations : Form
    {
        private bool isManager;
        public FormTblOperations(bool isManager)
        {
            InitializeComponent();
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            try
            {
                DataSetOperations changes = (DataSetOperations)dataSetOperations.GetChanges();
                if (changes == null)
                    return;
                // check for errors
                DataTable dt = changes.tblOperations.GetChanges();
                ...
                int numRows = tblOperationsTableAdapter.Update(changes);
                MessageBox.Show("Updated " + numRows + " rows", "Success");
                dataSetOperations.AcceptChanges();
            }
            catch (Exception ex)
            {
                MessageBox.Show("Error: " + ex.Message, "Erros",
                                MessageBoxButtons.OK, MessageBoxIcon.Error);
                dataSetOperations.RejectChanges();
            }
        }
    }
}

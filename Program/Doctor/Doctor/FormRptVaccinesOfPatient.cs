...
namespace Doctor
{
    public partial class FormRptVaccinesOfPatient : Form
    {
        ...
        private string patientRegVaccine;
        private string patientSciVaccine;
        private int lastRow = 0;
        private string saveColor = "";
        public FormRptVaccinesOfPatient(OleDbConnection dataConnection, bool isManager)
        {
            InitializeComponent();
        ...
        private void textboxPatientID_TextChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                datacommand.CommandText = "SELECT   patvacID, patvacDate, patvacName " +
                                          "FROM     tblPatientVaccines   " +
                                          "WHERE    patvacPatientID = " + patientID + " " +
                                          "ORDER BY patvacID";
                ...
                while (dataReader.Read())
                {
                    ...
                    patientRegVaccine = dataReader.GetString(2);
                    GetSciName();
                    counter++;
                    EditListView();
                }
        ...
        private void GetSciName()
        {
            try
            {
                OleDbCommand datacommand = new OleDbCommand();
                datacommand.Connection = dataConnection;
                datacommand.CommandText = "SELECT   vaccineSciName " +
                                          "FROM     tblVaccines   " +
                                          "WHERE    vaccineRegName = \"" + patientRegVaccine + "\"";
                OleDbDataReader dataReader = datacommand.ExecuteReader();
                if (dataReader.Read())
                {
                    patientSciVaccine = dataReader.GetString(0).ToString();
                }
                dataReader.Close();
            }
            catch (Exception ex)
        ...
        private void EditListView()
        {
            try
            {
                ...
                arr[2] = patientDate;
                arr[3] = patientRegVaccine;
                arr[4] = patientSciVaccine;
        ...
        private void FormRptVaccinesOfPatient_Load(object sender, EventArgs e)
        {

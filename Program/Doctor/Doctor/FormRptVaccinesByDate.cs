...
namespace Doctor
{
    public partial class FormRptVaccinesByDate : Form
    {
        ...
        public FormRptVaccinesByDate(OleDbConnection dataConnection, bool isManager)
        {
            InitializeComponent();
        ...
        private void buttonAdd_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                datacommand.CommandText = "SELECT   patvacPatientID, patvacName, patvacDate " +
                                          "FROM     tblPatientVaccines   " +
                                          "WHERE    patvacDate BETWEEN #" + dateTimePicker1.Value.ToShortDateString() + "#  AND #" + dateTimePicker2.Value.ToShortDateString() + "#  " +
                                          "ORDER BY patvacID";
        ...
        private void GetSci()
        {
            try
            {
                ...
                datacommand.CommandText = "SELECT   vaccineSciName " +
                                          "FROM     tblVaccines   " +
                                          "WHERE    vaccineRegName = \"" + patientReg + "\"";
                OleDbDataReader dataReader = datacommand.ExecuteReader();
                if (dataReader.Read())
                {

...
namespace Doctor
{
    public partial class FormRptTreatsByDate : Form
    {
        ...
        public FormRptTreatsByDate(OleDbConnection dataConnection, bool isManager)
        {
            InitializeComponent();
        ...
        private void buttonAdd_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                datacommand.CommandText = "SELECT   pattrtPatiId, pattrtName, pattrtDate " +
                                          "FROM     tblPatientTreats   " +
                                          "WHERE    pattrtDate BETWEEN #" + dateTimePicker1.Value.ToShortDateString() + "#  AND #" + dateTimePicker2.Value.ToShortDateString() + "#  " +
                                          "ORDER BY pattrtID";
        ...
        private void GetSci()
        {
            try
            {
                OleDbCommand datacommand = new OleDbCommand();
                datacommand.Connection = dataConnection;
                datacommand.CommandText = "SELECT   treatSciName " +
                                          "FROM     tblTreats   " +
                                          "WHERE    treatRegName = \"" + patientReg + "\"";
                OleDbDataReader dataReader = datacommand.ExecuteReader();
                if (dataReader.Read())
                {

...
namespace Doctor
{
    public partial class FormChartPatientAndDiseases : Form
    {
        private OleDbConnection dataConnection;
        private int counter;
        private string[] arrTypes;
        private int[] arrActual;
        public FormChartPatientAndDiseases(OleDbConnection dataConnection)
        {
            InitializeComponent();
        ...
        private void CountTypes()
        {
            ...
            {
                MessageBox.Show("Count tblDiseases failed " +
                                 ex.Message, "Errors",
        ...
        private void FillArrTypes()
        {
            ...
            {
                MessageBox.Show("Select tblDiseases failed " +
                                 ex.Message, "Errors",
                                 MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
        ...
        private void CountActual(int idx)
        {
            try
            {
                OleDbCommand datacommand = new OleDbCommand();
                datacommand.Connection = dataConnection;
                datacommand.CommandText = "SELECT  patdssName " +
                                          "FROM    tblPatientDiseases  " +
                                          "WHERE   patdssName = \"" + arrTypes[idx] + "\"";
                OleDbDataReader dataReader = datacommand.ExecuteReader();
                while (dataReader.Read())
                {
                    arrActual[idx]++;
        ...
        private void ShowChart()
        {
            ...
            try
            {
                ...
                {
                    chart1.Series["מחלות"].Points.AddXY(arrTypes[i], arrActual[i]);
                }
            }
            catch (Exception ex)
            {
        ...
        private void FormChartPatientAndDiseases_Load(object sender, EventArgs e)
        {

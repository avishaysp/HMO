...
namespace Doctor
{
    public partial class FormTblDiseases : Form
    {
        private bool isManager;
        public FormTblDiseases(bool isManager)
        {
            InitializeComponent();
        ...
        private void FormTblDiseases_Load(object sender, EventArgs e)
        {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            try
            {
                DataSetDiseases changes = (DataSetDiseases)dataSetDiseases.GetChanges();
                if (changes == null)
                    return;
                // check for errors
                DataTable dt = changes.tblDiseases.GetChanges();
                ...
                int numRows = tblDiseasesTableAdapter.Update(changes);
                MessageBox.Show("Updated " + numRows + " rows", "Success");
                dataSetDiseases.AcceptChanges();
            }
            catch (Exception ex)
            {
                MessageBox.Show("Error: " + ex.Message, "Erros",
                                MessageBoxButtons.OK, MessageBoxIcon.Error);
                dataSetDiseases.RejectChanges();
            }
        }
    }
}

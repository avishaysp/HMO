...
namespace Doctor
{
    public partial class FormRptDrugsByDate : Form
    {
        ...
        public FormRptDrugsByDate(OleDbConnection dataConnection, bool isManager)
        {
            InitializeComponent();
        ...
        private void buttonAdd_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                datacommand.CommandText = "SELECT   patdrgPatiID, patdrgName, patdrgDate " +
                                          "FROM     tblPatientDrugs   " +
                                          "WHERE    patdrgDate BETWEEN #" + dateTimePicker1.Value.ToShortDateString() + "#  AND #" + dateTimePicker2.Value.ToShortDateString() + "#  " +
                                          "ORDER BY patdrgID";
        ...
        private void GetSci()
        {
            try
            {
                OleDbCommand datacommand = new OleDbCommand();
                datacommand.Connection = dataConnection;
                datacommand.CommandText = "SELECT   drugSciName " +
                                          "FROM     tblDrugs   " +
                                          "WHERE    drugRegName = \"" + patientReg + "\"";
                OleDbDataReader dataReader = datacommand.ExecuteReader();
                if (dataReader.Read())
                {
